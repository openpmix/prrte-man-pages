...
if test -z "$srcfile"; then
    ...
    s/^std_major/PRTE_STD_MAJOR_VERSION/
    s/^std_minor/PRTE_STD_MINOR_VERSION/
    ...
    eval "$prte_vers"
        ...
        PRTE_STD_VERSION="$PRTE_STD_MAJOR_VERSION.$PRTE_STD_MINOR_VERSION"
...
case "$option" in
    ...
    --release-date)
        ...
        ;;
    --all)
        echo ${PRTE_VERSION} : ${PRTE_MAJOR_VERSION} : ${PRTE_MINOR_VERSION} : ${PRTE_RELEASE_VERSION} : ${PRTE_GREEK_VERSION} : ${PRTE_REPO_REV} : ${PPRTE_TARBALL_VERSION}
        ;;
    -h|--help)
    cat <<EOF
$0 <srcfile> <option>
